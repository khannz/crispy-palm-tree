...
func (modifyService *ModifyServiceEntity) isServicesIPsAndPortsEqual(serviceOne,
	...
	if serviceOne.Address != serviceTwo.Address {
	...
	for k := range serviceOne.ApplicationServers {
		if _, isFunded := serviceTwo.ApplicationServers[k]; !isFunded {
			logApplicationServerNotFound(serviceOne.IP, serviceOne.Port, serviceOne.ApplicationServers[k].IP, serviceOne.ApplicationServers[k].Port, modifyServiceName, id, modifyService.logging)
			return false
		}
	}

	for k := range serviceTwo.ApplicationServers {
		if _, isFunded := serviceOne.ApplicationServers[k]; !isFunded {
			logApplicationServerNotFound(serviceTwo.IP, serviceTwo.Port, serviceTwo.ApplicationServers[k].IP, serviceTwo.ApplicationServers[k].Port, modifyServiceName, id, modifyService.logging)
			return false
		}
	}
	return true
