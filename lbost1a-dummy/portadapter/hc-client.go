...
func convertRuntimeConfigToPbRuntimeConfig(runtimeConfig map[string]struct{}, id string) *transport.DummyRuntime {
	ed := &transport.EmptyData{}
	pbMap := make(map[string]*transport.EmptyData)
	for k := range runtimeConfig {
		pbMap[k] = ed
	}

	return &transport.DummyRuntime{
		Services: pbMap,
		Id:       id,
	}
}
